The Pawno embedded scripting language
====================================
Pawno is a simple, typeless, 32-bit extension language with a C-like syntax.
The Pawno compiler outputs P-code (or bytecode) that subsequently runs on an
...
Through the evolution of the Pawno toolkit, this README had steadily been
...
embed Pawno can be found in a separate document entitled "The Pawno booklet:
Implementer's Guide". To get the Pawno toolkit working on your system, please
(also) consult that document. To learn about the Pawno language, read the
document "The Pawno booklet: The Language". If you installed Pawno via the
...
documents from the web page devoted to Pawno:
        http://www.sdsls.com/pawno/

Below is a list of topics that this README covers, in this order:

o  Getting started
   How to get your first Pawno program running
...
Acknowledgements
================
See the "NOTICES" file for acknowledgments for parts of Pawno not written by
CompuPhase.
...
download the Pawno toolkit as a self-extracting setup file; this gives
...
Users of other operating systems should download either the "pawnokit.zip"
file or the "pawnokit.tgz" file ("Pawno toolkit" archives), because it
...
Pawno compiler will not be able to find its "include" files, for example.

You should also download the two documentation files "pawno-lang.pdf" and
...
Assuming that you have obtained (somehow) an executable version of the Pawno
compiler and the Pawno run-time, you should put it in a directory. However,
the Pawno compiler also needs to locate "include files". On many operating
systems, the Pawno compiler is able to automatically read these header files
from the directory "include" that is below the directory that the compiler is
in itself. Thus, if the Pawno compiler (the executable file) is in directory
"C:\WhatEver\Pawno\bin", I suggest that you create either the directory
"C:\WhatEver\Pawno\include" or "C:\WhatEver\Pawno\bin\include" and copy the
".inc" files there. If your operating system is not compatible with Win32 or
Linux, the Pawno compiler may not know how to locate the "include" directory
...
Many applications that use Pawno, run the Pawno compiler as a child process;
i.e. the Pawno compiler is often a separate, self-contained program. The
...
can call into the application. In other words, you might be using "pawnocc" as
is, but you won't be using "pawno" as is. This is why pawnorun is kept short
and dump, "pawnorun" is a program that is mostly developed in the Pawno manual to
show you what you should do at a minimum to embed Pawno into a program.
...
The Pawno toolkit comes with a CMake project file that builds the compiler, a
...
Pawno toolkit is installed, when you installed the self-extracting "setup" for
Microsoft Windows. When unpacking the Pawno source code from a "ZIP" or "TGZ"
...
2. Select for the source code directory, the "source" subdirectory in the
   ...
   For example, if you installed the toolkit in C:\Pawno, the source directory
   is C:\Pawno\source.
...
5. Click on the "Configure" button. After an initial configuration, you may
   ...
   may need adjustment, but in the case of Pawno, this is rarely needed. Click
...
The core and console functions are automatically registered to any Pawno
program by amx_Init().
